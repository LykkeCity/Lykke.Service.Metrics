...
using Lykke.Service.Metrics.Modules;
...
namespace Lykke.Service.Metrics
{
    public class Startup
    {
        ...
        public IServiceProvider ConfigureServices(IServiceCollection services)
        {
            ...
            {
                options.DefaultLykkeConfiguration("v1", "Metrics API");
            });
            ...
            builder.RegisterModule(new ServiceModule(appSettings.MetricsService, log));
        ...
        public void Configure(IApplicationBuilder app, IHostingEnvironment env, IApplicationLifetime appLifetime)
        {
            ...
            app.UseLykkeMiddleware("Metrics", ex => new {Message = "Technical problem"});
        ...
        private static ILog CreateLogWithSlack(IServiceCollection services, AppSettings settings)
        {
            ...
            var dbLogConnectionString = settings.MetricsService.Db.LogsConnString;

            // Creating azure storage logger, which logs own messages to concole log
            if (!string.IsNullOrEmpty(dbLogConnectionString) && !(dbLogConnectionString.StartsWith("${") && dbLogConnectionString.EndsWith("}")))
            {
                logToAzureStorage = new LykkeLogToAzureStorage("Lykke.Service.Metrics", new AzureTableStorage<LogEntity>(
                    dbLogConnectionString, "MetricsLog", logToConsole));
